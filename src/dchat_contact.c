 ...
 * =====================================================================================
 *
 *       Filename:  dchat_contact.c
 *
 *    Description:  This file contains functions concerning dchat contacts
 *                  for adding, deleting, searching, sending and receiving
 *                  contacts
 *
 *        Version:  1.0
...
/*! Send contacts in form of a "control/discover" pdu to the given contact
...
/*! Parses the contact information stored in the pdu, connects to the parsed contact
 *  (if unknown) sends him our contactlist and finally adds him as new contact.
 *  This is done for every successfully parsed contact.
 *  @param cnf: Pointer to global configuration holding the contactlist
...
/*! Checks if there are duplicate contacts in the contactlist and remove them
 *  if any exists
 *  @param cnf:   Pointer to global configuration holding the contactlist
...
int check_duplicates(dchat_conf_t* cnf, int n)
{
    ...
    int accept_contact =
        0;     //!< index of contact from whom we accepted a connection
...
/*! This function converts a contact to a string representation. The string
...
/*! Converts a contact string to a contact structure. The
 *  string has to be in the form of: <ip> <port>\n
 *  This function is the opposite of the function  contact_to_string.
 *  @param contact: Pointer to contact that should be converted to a string
 ...
 *  @param the new size for the contactlist
 *  @return 0 if no error occurs, -1 if malloc fails, -2 if newsize is less than 1
 */
...
/*! Adds a new client (socket descriptor and socket address) to the contact list
 *  defined in dchat_conf_t structure, which is given as argument.
...
/*! Deletes a contact from the contact list holded by the given dchat_conf_t structure
 *  @param cnf: Pointer to dchat_conf_t structure holding the contact list
...
/*! Searches for a contact in the contactlist and returns its index in the
 *  contactlist. To find a contact, its socket address and listening port will
 *  be used
 *  @param cnf:     Pointer to global configuration holding the contactlist
